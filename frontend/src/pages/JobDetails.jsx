...
const JobDetails = () => {
    ...
    const fetchEvents = async()=> {
        try {
            const {data} = await axios.get(backendUrl+`/events/${id}`,{headers:{token}})
            if(data.success) {
    ...
    const getJob = async()=> {
        try {
            const {data} = await axios.get(backendUrl+`/job/single/${id}`,{headers:{token}})
    ...
    const addEvent = async()=> {
        try {
            const {data} = await axios.post(backendUrl+`/events/${id}`,{
                type:eventType,
                date: eventDate,
                note: eventNote
            },{headers:{token}})
